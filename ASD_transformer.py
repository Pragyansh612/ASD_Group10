...
from model_transformer.Model import ASD_Model
...
class ASD(nn.Module):
    def __init__(self, lr=0.001, lrDecay=0.95, **kwargs):
        ...
        print(time.strftime("%m-%d %H:%M:%S") + " Model para number = %.2f" % (sum(p.numel() for p in self.model.parameters()) / 1e6))
    ...
    def loadParameters(self, path):
        ...
        for name, param in loadedState.items():
            ...
            if selfState[name].size() != para.size():
                print("Wrong parameter length: %s, model: %s, loaded: %s" % (origName, selfState[name].size(), para.size()))
                continue
